...
with open(r'C:\Users\Lauta\OneDrive\Escritorio\Prone\PropiedadesNoDispo.json', 'r', encoding='utf-8') as file:
    data = json.load(file)
...
# Recorrer cada objeto en la lista del JSON
for obj in data:
    prop_count += 1  # Contador de propiedades
    ...
    if branch:  # Verificar que branch no esté vacío
        branch_count += 1
    ...
    if location:  # Verificar que location no esté vacío
        location_count += 1
        location_data = {
            'property_id': obj.get('id', None),  # Relacionar con propiedad
    ...
    if producer:  # Verificar que producer no esté vacío
        producer_count += 1
        producer_data = {
            'property_id': obj.get('id', None),  # Relacionar con propiedad
    ...
    if type_info:  # Verificar que type no esté vacío
        type_count += 1
        type_data = {
            'property_id': obj.get('id', None),  # Relacionar con propiedad
    ...
    if operations:  # Verificar que operations no esté vacío
    ...
    if property_owners:  # Verificar que property_owners no esté vacío
...
with pd.ExcelWriter('Propiedades_Todo_en_Uno_NoDispo.xlsx') as writer:
...
# Imprimir contadores de depuración
print(f"Propiedades procesadas: {prop_count}")
print(f"Sucursales procesadas: {branch_count}")
print(f"Ubicaciones procesadas: {location_count}")
print(f"Productores procesados: {producer_count}")
print(f"Tipos procesados: {type_count}")
print(f"Operaciones procesadas: {operation_count}")
print(f"Propietarios procesados: {owner_count}")
